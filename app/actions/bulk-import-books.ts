...
export async function bulkImportBookObjects(bookObjects: any[]): Promise<ImportResult> {
  ...
  try {
    ...
    result.errorDetails?.push(
      `Bulk import error: ${error instanceof Error ? error.message : String(error)}`
    )
    return result
  }
}
    ...
    if (duplicates.length > 0) {
      try {
        ...
        if (duplicateBooks && duplicateBooks.length > 0) {
          ...
          duplicateBooks.forEach(
            (book: { title: string; isbn10: string | null; isbn13: string | null }) => {
    ...
    for (const book of books) {
      try {
        ...
        if (book.binding) {
          ...
          let formatName = 'Print' // default format
          if (
            book.binding.toLowerCase().includes('ebook') ||
            book.binding.toLowerCase().includes('kindle')
          ) {
            formatName = 'Digital'
          } else if (book.binding.toLowerCase().includes('audio')) {
            formatName = 'Audio'
        ...
        if (book.authors && Array.isArray(book.authors) && book.authors.length > 0) {
          console.log(
            `Processing ${book.authors.length} authors for book "${book.title}":`,
            book.authors
          )
          for (const authorName of book.authors) {
            // Skip empty or invalid author names
            ...
            if (existingAuthor) {
              ...
              if (authorError) {
                ...
                result.errors++
                result.errorDetails?.push(
                  `Failed to create author "${cleanAuthorName}" for book ${book.title}: ${authorError.message}`
                )
              } else if (newAuthor) {
                console.log(`Successfully created author: ${cleanAuthorName} (ID: ${newAuthor.id})`)
                authorIds.push(newAuthor.id)
            ...
            for (let i = 0; i < authorIds.length; i++) {
              ...
              try {
                ...
                if (linkError) {
                  ...
                  if (insertError) {
                    console.warn(
                      `Failed to link author ${authId} to book "${book.title}":`,
                      insertError
                    )
                    result.errorDetails?.push(
                      `Failed to link author to book "${book.title}": ${insertError.message}`
                    )
                  } else {
                    linkedCount++
                  }
