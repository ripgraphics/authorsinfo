  ...
  try {
      ...
      if (!entityType && albumData.entity_type) {
        ...
        const albumsWithImages = await Promise.all(
          albums.map(async (alb) => {
            ...
            const imageIds = (albumImages as Array<{ image_id: string | null }>)
              .map((ai) => ai.image_id)
              .filter(Boolean)
            ...
            const imageMap = new Map(
              (allImages || []).map((img: { id: string; [key: string]: any }) => [img.id, img])
            )
            const imagesWithDetails = (
              albumImages as Array<{ image_id: string | null; [key: string]: any }>
            )
              .map((albumImage) => {
    ...
    try {
      ...
      if (albumPurpose) {
        ...
        console.log(
          `🔍 Querying albums for: entityId=${entityId}, entityType=${entityType}, albumPurpose=${albumPurpose}, expectedName="${expectedAlbumName}"`
        )
        query = query.eq('name', expectedAlbumName)
      } else {
        console.log(
          `🔍 Querying albums for: entityId=${entityId}, entityType=${entityType}, albumPurpose=ALL`
        )
      }
        ...
        (albums || []).map(
          async (album: {
              ...
              if (!actualEntityType) {
                ...
                if (sampleImageData?.images?.storage_path) {
                  ...
                  if (path.includes('book_')) actualEntityType = 'book'
                    ...
                    path.includes('user_') ||
                    path.includes('user_photos') ||
                    path.includes('user_album')
                  )
            ...
            `
              ...
              if (!albumImages || albumImages.length === 0) {
                console.log(
                  `⚠️ No album_images found for album ${album.id}, returning empty images array`
                )
                return {
                  ...album,
                  images: [],
              ...
              if (albumImages && albumImages.length > 0) {
                ...
                const coverImages = (
                  albumImages as Array<{ is_cover?: boolean; [key: string]: any }>
                ).filter((ai) => ai.is_cover === true)
                if (coverImages.length > 1) {
                  ...
                  const mostRecentImage = (
                    albumImages as Array<{ created_at?: string; id?: string; [key: string]: any }>
                  ).sort(
                    (a, b) =>
                  ...
                  if (mostRecentImage) {
                    await (supabase.from('album_images') as any)
                      ...
                      .eq(
                        ...
                        albumImages as Array<{
                          is_cover?: boolean
                          id?: string
                          [key: string]: any
                        }>
                      )
                      .forEach((ai: { is_cover?: boolean; id?: string; [key: string]: any }) => {
            ...
            `
              ...
              const imagesWithDetails = (
                ...
                if (
                  imageDetails.url &&
                  (imageDetails.url.startsWith('blob:') || imageDetails.url.startsWith('data:'))
                ) {
                  console.warn(
                    `⚠️ Image ${albumImage.image_id} has invalid URL (blob/data): ${imageDetails.url.substring(0, 50)}...`
                  )
                }
              ...
              console.log(
                `🖼️ Album ${album.id} (${album.name}): ${validImages.length} valid images, ${invalidImages.length} invalid/missing images`
              )
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📥 Album addition request:`, {
      entityId,
      entityType,
      albumPurpose,
      imageId,
    ...
    console.log(
      `✅ Image verified in database: ${(verifyImage as any)?.id}, URL: ${(verifyImage as any)?.url}`
    )
    ...
    const { data: existingAlbum, error: searchError } = await supabase
      .from('photo_albums')
    ...
    if (!existingAlbum) {
      ...
      console.log(`✅ Album created: ${albumId}`, {
        name: (newAlbum as any).name,
        entity_id: (newAlbum as any).entity_id,
        entity_type: (newAlbum as any).entity_type,
      })
    } else {
    ...
    const { error: updateMetadataError } = await (supabase.from('photo_albums') as any)
      .update({
        metadata: {
    ...
    if (entityType === 'publisher' && albumPurpose === 'avatar' && (isCover || isFeatured)) {
      console.log(
        `🔄 Updating publisher_image_id for publisher ${entityId} with image ${finalImageId}`
      )
      const { error: publisherUpdateError } = await (supabase.from('publishers') as any)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (altText || caption || metadata) {
      const { error: imageUpdateError } = await (supabase.from('images') as any)
        .update({
          alt_text: altText,
          ...(caption && { caption: caption }),
    ...
    if (Object.keys(updateData).length > 0) {
      const { error: albumImageUpdateError } = await (supabase.from('album_images') as any)
        .update(updateData)
        .eq('album_id', albumId)
        .eq('image_id', imageId)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error: removeError } = await (supabase.from('album_images') as any)
      .delete()
    ...
    await (supabase.from('photo_albums') as any)
      .update({
        metadata: {
