...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (userRole) {
    data = (data || []).filter(
      (a: any) =>
        !a.visible_to_roles ||
        a.visible_to_roles.length === 0 ||
        a.visible_to_roles.includes(userRole)
    )
  }
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body.title || !body.body || !body.created_by)
    return NextResponse.json({ error: 'Missing title, body, or created_by' }, { status: 400 })
...
export async function GET_readers(
  req: NextRequest,
...
export async function POST_readers(
  req: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  const supabase = createClient()
  const body = await req.json()
  if (!body.announcement_id || !body.user_id)
    return NextResponse.json({ error: 'Missing announcement_id or user_id' }, { status: 400 })
  const { data, error } = await supabase
    .from('group_announcement_reads')
    .insert([{ ...body, group_id: id }])
    .select()
