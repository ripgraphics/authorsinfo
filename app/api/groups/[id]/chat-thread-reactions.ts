...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body.thread_id || !body.user_id || !body.emoji) return NextResponse.json({ error: 'Missing thread_id, user_id, or emoji' }, { status: 400 });
  const { data, error } = await supabase
    .from('group_chat_thread_reactions')
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!threadId || !userId || !emoji) return NextResponse.json({ error: 'Missing thread_id, user_id, or emoji' }, { status: 400 });
