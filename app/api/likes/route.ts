  ...
  try {
    ...
    if (existingLike) {
      ...
      if (deleteLikeActivityError) {
        console.warn(
          'Warning: Failed to remove like activity from timeline:',
          deleteLikeActivityError
        )
      }
      ...
      const { data: newLike, error: insertError } = await (supabase.from('activity_likes') as any)
        .insert([
          {
            activity_id,
            user_id: user.id,
