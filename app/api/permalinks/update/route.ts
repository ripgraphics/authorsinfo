...
export async function POST(request: NextRequest) {
  try {
    ...
    const isValidFormat =
      /^[a-z0-9-]{3,100}$/.test(newPermalink) &&
    ...
    if (!isValidFormat) {
      return NextResponse.json({
        success: false,
        error:
          'Invalid permalink format. Must be 3-100 characters, lowercase letters, numbers, and hyphens only.',
      })
    }
    ...
    if (entityType === 'user' && entityId !== user.id) {
      return NextResponse.json({ error: 'You can only update your own permalink' }, { status: 403 })
    }
    ...
    const { data: existingEntity, error: checkError } = await (supabase.from(tableName) as any)
      .select('id')
    ...
    if (checkError && checkError.code !== 'PGRST116') {
      ...
      return NextResponse.json({ error: 'Failed to check permalink availability' }, { status: 500 })
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to update permalink' }, { status: 500 })
    }
