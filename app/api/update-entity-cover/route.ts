...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Updating ${tableName} table with ${updateColumn} = ${imageId} where id = ${entityId}`)
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: `Failed to update ${entityType} profile: ${updateError.message}` }, { status: 500 })
    }
