...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: userProfile, error: profileError } = await (supabase.from('users') as any)
    ...
    const { data: booksRead, error: booksError } = await (supabase.from('reading_progress') as any)
    ...
    // Get friends count
    const { data: friends, error: friendsError } = await (supabase.from('user_friends') as any)
      .select('id', { count: 'exact' })
      .eq('user_id', userId)
      .eq('status', 'accepted')
    ...
    const { data: reverseFriends, error: reverseFriendsError } = await (
    ...
    const { data: currentlyReading, error: readingError } = await (
    ...
    // Get recent photos
    const { data: recentPhotos, error: photosError } = await (supabase.from('images') as any)
