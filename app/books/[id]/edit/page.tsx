    ...
    try {
      ...
      if (coverImage) {
        try {
          ...
          if (!uploadResult.imageId) {
            throw new Error(
              'Image uploaded successfully but no image ID was returned from the database'
            )
          }
          ...
          if (book.cover_image_id) {
            const { error: imageUpdateError } = await (supabaseClient.from('images') as any)
      ...
      try {
        ...
        if (selectError) {
          ...
          setError(
            `Error finding book with ID ${bookId}: ${selectError.message || JSON.stringify(selectError)}`
          )
          toast({
  ...
  useEffect(() => {
    async function fetchBookData() {
      try {
        ...
        console.log(
          'Setting featured state to:',
          bookData.featured === true || bookData.featured === 'true'
        )
      } catch (error) {
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Search for authors by name. The first author will be set as the primary
                      author.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-sm text-muted-foreground mt-1">
                      Search for publishers by name. The first publisher will be set as the primary
                      publisher.
                    </p>
                  </div>
