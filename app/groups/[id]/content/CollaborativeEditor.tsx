...
export function CollaborativeEditor({
  ...
  useEffect(() => {
    ...
    channel.current = supabase.current
      .channel(`group_content_${contentId}_presence`, {
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'group_content', filter: `id=eq.${contentId}` },
        (payload: any) => {
  ...
  return (
    <div className="space-y-2">
      ...
      {users.map((u: any) =>
        ...
        u.selectionEnd !== undefined &&
        u.selectionStart !== u.selectionEnd ? (
          <div key={u.user_id + '-sel'} className="flex items-center gap-2 mb-1">
            <span
              className="inline-block w-2 h-2 rounded-full"
              style={{ background: getColorFromId(u.user_id) }}
            ></span>
            <span className="text-xs" style={{ color: getColorFromId(u.user_id) }}>
              {u.name}
            </span>
            ...
            <div
              className="h-2 rounded-sm opacity-40"
              style={{
                background: getColorFromId(u.user_id),
                width: Math.max(20, Math.abs(u.selectionEnd - u.selectionStart) * 2),
              }}
            />
          </div>
        ) : null
      ...
      <div className="flex gap-2 mb-1">
        ...
        {users.map((u: any) => (
          <span key={u.user_id} className="flex items-center gap-1">
            ...
            {u.typing && (
              <span className="ml-1 text-xs text-blue-500 animate-pulse">is typing...</span>
            )}
          </span>
        ))}
      </div>
