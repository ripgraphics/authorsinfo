...
export default function EventCard({ event, className = '', isCompact = false }: EventCardProps) {
  const formattedStartDate = event.start_date ? format(new Date(event.start_date), 'MMM d, yyyy') : 'TBD';
  ...
  const imageUrl = event.cover_image?.url || event.event_image?.url || '/images/event-placeholder.jpg';
