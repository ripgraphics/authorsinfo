...
import { Users, UserPlus, Check, X, Loader2, Clock } from 'lucide-react'
    ...
    return (
      <Card className={cn('friend-requests-widget', className)}>
        <CardContent className={cn('friend-requests-widget__loading-content', 'p-4')}>
          <div
            className={cn(
              'friend-requests-widget__loading-spinner',
  ...
  return (
    <Card className={cn('friend-requests-widget', 'w-full', className)}>
      <CardHeader className={cn('friend-requests-widget__header', 'pb-3 px-4 pt-4')}>
        <CardTitle
          className={cn(
            'friend-requests-widget__title',
            'flex items-center justify-between text-sm gap-2'
          )}
        >
          <div
            className={cn(
              'friend-requests-widget__title-content',
          ...
          <Badge
            variant="secondary"
            className={cn('friend-requests-widget__badge', 'text-xs flex-shrink-0')}
          >
            {requests.length}
          </Badge>
        </CardTitle>
      ...
      <CardContent className={cn('friend-requests-widget__content', 'space-y-3 px-4 pb-4')}>
        {requests.map((request) => (
          ...
          >
            <Avatar
              className={cn('friend-requests-widget__request-avatar', 'h-10 w-10 flex-shrink-0')}
            >
              <AvatarImage
            ...
            <div
              className={cn(
                'friend-requests-widget__request-details',
            ...
            >
              <Link
                ...
                className={cn(
                  'friend-requests-widget__request-name',
                  'font-medium text-sm hover:underline block truncate'
                )}
              >
                {request.user.name}
              </Link>
              <p
                className={cn(
                  'friend-requests-widget__request-time',
            ...
            <div
              className={cn(
                'friend-requests-widget__request-actions',
            ...
            >
              <Button
                ...
                className={cn(
                  'friend-requests-widget__reject-button',
              ...
              <Button
                ...
                disabled={processingRequest === request.id}
                className={cn(
                  'friend-requests-widget__accept-button',
        ...
        {requests.length > 0 && (
          <div className={cn('friend-requests-widget__footer', 'pt-3 border-t')}>
            <Button
              variant="outline"
              size="sm"
              className={cn('friend-requests-widget__view-all-button', 'w-full')}
              asChild
            >
