...
export function usePhotoGalleryAccessibility(
  ...
  const { onImageSelect, onImageDelete, onImageEdit, onImageShare, onFullscreenToggle } = options
  ...
  const handleKeyDown = useCallback(
    ...
    [
      accessibilityState.focusedIndex,
      images,
      onImageSelect,
      onImageDelete,
      onImageEdit,
  ...
  const getImageAriaAttributes = useCallback(
    (image: AlbumImage, index: number) => {
      return {
        role: 'button',
        tabIndex: 0,
