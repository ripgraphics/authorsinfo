...
export function usePhotoGalleryAlbum(options: AlbumOptions = {}) {
  const { autoUpdateMetadata = true, maxImages } = options
  ...
  const createAlbum = useCallback(
    async (name: string, description?: string, coverImageId?: string): Promise<Album> => {
      setAlbumState((prev) => ({
        ...prev,
        isLoading: true,
        error: null,
  ...
  const updateAlbum = useCallback(
    async (albumId: string, updates: Partial<Album>): Promise<Album> => {
      setAlbumState((prev) => ({
        ...prev,
        isLoading: true,
        error: null,
  ...
  // Get album images
  const getAlbumImages = useCallback(
    async (albumId: string, page = 1, pageSize = 20): Promise<AlbumImage[]> => {
      setAlbumState((prev) => ({
        ...prev,
        isLoading: true,
        error: null,
  ...
  const addImageToAlbum = useCallback(
    async (albumId: string, imageId: string, displayOrder?: number): Promise<void> => {
      try {
        ...
        // Add image to album
        const { error: addError } = await (supabase.from('album_images') as any).insert({
          album_id: albumId,
          image_id: imageId,
  ...
  const removeImageFromAlbum = useCallback(
    async (albumId: string, imageId: string): Promise<void> => {
      try {
        ...
        throw new Error(
          error instanceof Error ? error.message : 'Failed to remove image from album'
        )
      }
    },
    [supabase, autoUpdateMetadata]
  ...
  const reorderImages = useCallback(
    async (albumId: string, imageIds: string[]): Promise<void> => {
      try {
        ...
        const { error } = await (supabase.from('album_images') as any).upsert(updates)
  ...
  const updateAlbumMetadata = useCallback(
    async (albumId: string): Promise<void> => {
      try {
        // Get album images
        ...
        const totalSize = images.reduce(
          (sum: number, item: any) => sum + ((item as any).images?.size ?? 0),
          0
        )
        const lastModified = new Date().toISOString()

        // Update album metadata
        const { error: updateError } = await (supabase.from('albums') as any)
          .update({
            metadata: {
