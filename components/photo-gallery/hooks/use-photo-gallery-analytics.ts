...
export function usePhotoGalleryAnalytics(albumId: string) {
  ...
  // Track image click
  const trackImageClick = useCallback(async (imageId: string) => {
    await trackEvent({
      type: 'click',
      albumId,
      imageId,
  ...
  // Track image share
  const trackShare = useCallback(async (platform: string, imageId?: string) => {
    await trackEvent({
      type: 'share',
      albumId,
      imageId,
  ...
  const trackDownload = useCallback(async (imageId: string) => {
    await trackEvent({
      type: 'download',
      albumId,
      imageId,
  ...
  // Track image like
  const trackLike = useCallback(async (imageId: string) => {
    await trackEvent({
      type: 'like',
      albumId,
      imageId,
  ...
  // Track delete
  const trackDelete = useCallback(async (imageId: string) => {
    await trackEvent({
      type: 'delete',
      albumId,
      imageId,
  ...
  // Track edit
  const trackEdit = useCallback(async (imageId: string, editType: string) => {
    await trackEvent({
      type: 'edit',
      albumId,
      imageId,
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const totalRevenue = albumImages?.reduce((sum, ai) => sum + (ai.revenue_generated || 0), 0) || 0
      const engagementRate = albumImages?.length ? (totalViews / albumImages.length) * 100 : 0

      // Get top performing content
      const topContent = albumImages
