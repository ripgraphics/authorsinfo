...
export function usePhotoGalleryMonetization(albumId: string) {
  ...
  const trackEvent = useCallback(async (event: MonetizationEvent) => {
    try {
  ...
  const trackUpload = useCallback(async (imageId: string) => {
  ...
  const enablePremiumContent = useCallback(async (imageId: string, price: number) => {
    try {
      const { error } = await supabase
        .from('album_images')
        .update({
  ...
  const processSubscription = useCallback(async (tier: string, amount: number) => {
    try {
      await trackEvent({
        type: 'subscription',
        albumId,
  ...
  // Process tip payment
  const processTip = useCallback(async (amount: number, message?: string) => {
    try {
      await trackEvent({
        type: 'tip',
        albumId,
  ...
  // Process ad revenue
  const processAdRevenue = useCallback(async (amount: number, adType: string) => {
    try {
      await trackEvent({
        type: 'ad_revenue',
        albumId,
  ...
  // Process sponsorship
  const processSponsorship = useCallback(async (amount: number, sponsor: string) => {
    try {
      await trackEvent({
        type: 'sponsorship',
        albumId,
  ...
  const processMerchandiseSale = useCallback(async (amount: number, product: string) => {
    try {
      await trackEvent({
        type: 'merchandise',
        albumId,
  ...
  const loadMonetization = useCallback(async () => {
    ...
    try {
      ...
      const totalEarnings = albumImages?.reduce((sum, ai) => sum + (ai.revenue_generated || 0), 0) || 0
      ...
      // Get revenue sources
      const revenueSources = albumImages
  ...
  const updateSettings = useCallback(async (newSettings: Partial<MonetizationSettings>) => {
    try {
  ...
  const generateRevenueReport = useCallback(async (period: 'daily' | 'weekly' | 'monthly' | 'yearly') => {
    try {
