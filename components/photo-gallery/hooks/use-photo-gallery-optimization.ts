...
export function usePhotoGalleryOptimization(options: OptimizationOptions = {}) {
  ...
  const optimizeImage = useCallback(
    async (imageUrl: string, width: number, height: number): Promise<OptimizedImage> => {
  ...
  const getOptimizedImageUrl = useCallback(
    async (image: AlbumImage, width: number, height: number): Promise<string> => {
  ...
  const preloadImages = useCallback(
    async (images: AlbumImage[], width: number, height: number) => {
      const preloadPromises = images.map((image) => getOptimizedImageUrl(image, width, height))
