...
export function usePhotoGalleryShare() {
  ...
  const generateShareUrl = useCallback((image: AlbumImage, options: ShareOptions) => {
    ...
    const title = encodeURIComponent(
      options.title || (image as any).altText || (image as any).alt_text || ''
    )
    const description = encodeURIComponent(
      options.description || (image as any).caption || (image as any).description || ''
    )
  ...
  const shareImage = useCallback(
    async (image: AlbumImage, options: ShareOptions) => {
      setShareState({
        isSharing: true,
        error: null,
        shareUrl: null,
      ...
      try {
        ...
        // Track share event
        const { error: trackError } = await (supabase.from('image_shares') as any).insert({
          image_id: image.id,
