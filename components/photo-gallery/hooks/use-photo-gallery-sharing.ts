...
export function usePhotoGallerySharing(options: ShareOptions = {}) {
  ...
  const generateShareLink = useCallback(
    async (imageId: string, customOptions?: ShareOptions): Promise<ShareLink> => {
      setShareState((prev) => ({
        ...prev,
        isLoading: true,
        error: null,
  ...
  const getShareLink = useCallback(
    async (token: string): Promise<ShareLink> => {
      setShareState((prev) => ({
        ...prev,
        isLoading: true,
        error: null,
  ...
  // Get shared image
  const getSharedImage = useCallback(
    async (token: string): Promise<AlbumImageLegacy> => {
      try {
        // Get share link
  ...
  const verifyShareLinkPassword = useCallback(
