...
export function usePhotoGallery({
    ...
    async (pageNum: number) => {
      try {
        ...
        const formattedImages: AlbumImageLegacy[] = data.map((item: any) => ({
          ...
          tags:
            item.image_tag_mappings?.map((mapping: any) => ({
  ...
  const handleImageReorder = useCallback(
    async (imageId: number, newOrder: number) => {
      try {
        setIsLoading(true)
        setError(null)
  ...
  const handleImageTag = useCallback(
    async (imageId: number, tags: string[]) => {
      try {
        ...
        const tagPromises = tags.map(async (tagName) => {
          ...
          // Create mapping
          const { error: mappingError } = await supabase.from('image_tag_mappings').insert({
            image_id: imageId,
            tag_id: tagData.id,
          })
        ...
        setImages((prev) =>
          prev.map((img) => (Number(img.id) === imageId ? { ...img, tags: newTags } : img))
        )
      } catch (err) {
