...
export type EntityType = 'user' | 'book' | 'author' | 'publisher' | 'group' | 'activity' | 'event' | 'photo' | 'album' | 'review' | 'comment'
...
export interface EngagementContextValue extends EngagementContextState {
  ...
  setReaction: (entityId: string, entityType: EntityType, reactionType: ReactionType) => Promise<boolean>
  ...
  // Bulk operations
  batchUpdateEngagement: (updates: Array<{ entityId: string; entityType: EntityType; updates: Partial<EngagementState> }>) => void
  ...
  getEntityStats: (entityId: string, entityType: EntityType) => { reactionCount: number; commentCount: number; shareCount: number }
...
type EngagementActionType = 
  | { type: 'SET_ENTITY_ENGAGEMENT'; payload: { entityId: string; entityType: EntityType; state: Partial<EngagementState> } }
  | { type: 'SET_REACTION'; payload: { entityId: string; entityType: EntityType; reactionType: ReactionType | null } }
  | { type: 'INCREMENT_COUNT'; payload: { entityId: string; entityType: EntityType; countType: keyof Pick<EngagementState, 'reactionCount' | 'commentCount' | 'shareCount' | 'bookmarkCount' | 'viewCount'> } }
  | { type: 'DECREMENT_COUNT'; payload: { entityId: string; entityType: EntityType; countType: keyof Pick<EngagementState, 'reactionCount' | 'commentCount' | 'shareCount' | 'bookmarkCount' | 'viewCount'> } }
  | { type: 'SET_LOADING'; payload: { entityId: string; entityType: EntityType; loading: boolean } }
  | { type: 'SET_ERROR'; payload: { entityId: string; entityType: EntityType; error: string | null } }
  ...
  switch (action.type) {
    ...
    case 'SET_REACTION': {
      ...
      updatedEntities.set(key, {
        ...
        reactionCount: reactionType ? existing.reactionCount + 1 : Math.max(0, existing.reactionCount - 1)
      })
...
export function EngagementProvider({ children }: EngagementProviderProps) {
  ...
  const getEngagement = useCallback((entityId: string, entityType: EntityType): EngagementState | null => {
    const key = `${entityType}:${entityId}`
    return state.entities.get(key) || null
  }, [state.entities])
  
  const setReaction = useCallback(async (entityId: string, entityType: EntityType, reactionType: ReactionType): Promise<boolean> => {
    if (!user) {
      toast({
    ...
    try {
      ...
      if (result.success) {
        ...
        if (currentReaction === reactionType) {
          // Removing reaction
          dispatch({ type: 'SET_REACTION', payload: { entityId, entityType, reactionType: null } })
          toast({
      ...
      dispatch({ 
        type: 'SET_ERROR', 
        payload: { entityId, entityType, error: error instanceof Error ? error.message : 'Unknown error' } 
      })
  ...
  const removeReaction = useCallback(async (entityId: string, entityType: EntityType): Promise<boolean> => {
  ...
  const addComment = useCallback(async (entityId: string, entityType: EntityType, commentText: string): Promise<boolean> => {
    if (!user) {
      toast({
        title: "Authentication required",
    ...
    try {
      ...
      if (result.success) {
        dispatch({ type: 'INCREMENT_COUNT', payload: { entityId, entityType, countType: 'commentCount' } })
        dispatch({ 
  ...
  const shareEntity = useCallback(async (entityId: string, entityType: EntityType): Promise<boolean> => {
    if (!user) {
      toast({
        title: "Authentication required",
    ...
    try {
      ...
      if (result.success) {
        dispatch({ type: 'INCREMENT_COUNT', payload: { entityId, entityType, countType: 'shareCount' } })
        dispatch({ 
          type: 'SET_ENTITY_ENGAGEMENT', 
  ...
  const bookmarkEntity = useCallback(async (entityId: string, entityType: EntityType): Promise<boolean> => {
    if (!user) {
      toast({
        title: "Authentication required",
    ...
    try {
      ...
      if (result.success) {
        ...
        if (isBookmarked) {
          dispatch({ type: 'INCREMENT_COUNT', payload: { entityId, entityType, countType: 'bookmarkCount' } })
        } else {
          dispatch({ type: 'DECREMENT_COUNT', payload: { entityId, entityType, countType: 'bookmarkCount' } })
        }
        ...
        toast({
          ...
          description: isBookmarked ? "Content added to your bookmarks" : "Content removed from bookmarks",
          variant: "default"
        })
  ...
  const viewEntity = useCallback(async (entityId: string, entityType: EntityType): Promise<boolean> => {
    try {
      ...
      if (response.ok) {
        dispatch({ type: 'INCREMENT_COUNT', payload: { entityId, entityType, countType: 'viewCount' } })
        dispatch({ 
          type: 'SET_ENTITY_ENGAGEMENT', 
  ...
  const batchUpdateEngagement = useCallback((updates: Array<{ entityId: string; entityType: EntityType; updates: Partial<EngagementState> }>) => {
  ...
  const isEntityEngaged = useCallback((entityId: string, entityType: EntityType): boolean => {
    const engagement = getEngagement(entityId, entityType)
    return !!(engagement?.userReaction || engagement?.userHasCommented || engagement?.userHasShared || engagement?.userHasBookmarked)
  }, [getEngagement])
  
  const getEntityReaction = useCallback((entityId: string, entityType: EntityType): ReactionType | null => {
    const engagement = getEngagement(entityId, entityType)
    return engagement?.userReaction || null
  }, [getEngagement])
  
  const getEntityStats = useCallback((entityId: string, entityType: EntityType) => {
    const engagement = getEngagement(entityId, entityType)
    return {
...
export function useEntityEngagement(entityId: string, entityType: EntityType) {
  ...
  return {
    ...context,
    engagement,
    setReaction: (reactionType: ReactionType) => context.setReaction(entityId, entityType, reactionType),
