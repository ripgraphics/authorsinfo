...
| `[>]` Complete group conversation creation, roles, avatars, and history policy | `IN PROGRESS` | Transport and membership slices exist; creation/roles/avatar policy incomplete | Implement group creation/role/settings contract |
...
| `[ ]` Entity message authorization and privacy matrix | `NOT STARTED` | Target resolver tests cover fail-closed behavior only | Add owner/non-owner/admin/blocked/private entity cases |
...
| `[ ]` Encrypted/access-controlled private media boundary | `NOT STARTED` | Matrix/E2EE remains gated | Complete provider/security decision |

### Phase 5: Notifications, Presence, Safety, and Privacy — NOT STARTED

| Item | Status | Evidence | Next action |
| --- | --- | --- | --- |
| `[ ]` In-app/push notifications and safe previews | `NOT STARTED` | Existing dispatcher is not a complete Messenger notification contract | Define preferences/outbox acceptance tests |
| `[>]` Presence, typing expiration, privacy, account-switch cleanup | `IN PROGRESS` | Typing indicator exists; full presence/privacy parity absent | Complete presence and account-isolation tests |
| `[ ]` Request/block/report/rate-limit workflows | `NOT STARTED` | No complete unified safety workflow | Add authenticated safety route contracts |
...
| `[>]` Duplicate message list/composer implementations | Shared `DirectMessageList` and `ChatComposer` contracts | Inventory in Phase 0 | All callers migrated and focused tests pass |
