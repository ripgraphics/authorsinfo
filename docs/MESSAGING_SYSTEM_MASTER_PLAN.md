...
| `[>]` Unauthorized/blocked/removed/suspended/event/forged cases fail closed | `IN PROGRESS` | Request and Restrict route tests cover forged/nonparticipant access; event target owner/non-owner/admin/private/public/forged matrix passes 16/16; direct-message RLS denials now return sanitized 403 responses with focused send/error tests passing 15/15; live group acceptance now proves active-member access, suspended/nonmember denial, removed-member denial for member list/history/send, active-member continuity, event-channel denial, and forged-sender rejection. Unreachable external rate-limit providers now fail open within 750ms instead of blocking valid sends. Direct blocked/removed/suspended acceptance remains incomplete | Verify live direct blocked/removed/suspended cases and broader authenticated browser acceptance |
