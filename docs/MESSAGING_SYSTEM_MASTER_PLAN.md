...
| Item | Status | Evidence | Next action |
| --- | --- | --- | --- |
| `[>]` Two authenticated accounts exchange direct messages without reload | `IN PROGRESS` | Sequential Chrome acceptance now passes after the login server-session readiness fix: Sam Smith logged in through the user card, opened Bob Brown's Messenger thread, sent `Sequential acceptance check 2026-09-23 Sam to Bob`, signed out, Bob Brown logged in through the user card, loaded the same persisted message, and sent `Sequential acceptance reply 2026-09-23 Bob to Sam`; both messages rendered with normal message actions. The proxy now refreshes Supabase cookies on normal application/API requests, preventing the prior post-login `401` cascade; a fresh Bob session loaded `/messages` with Bob's header and 9 conversation rows including Sam. This proves persisted two-account send/receive, account switching, and stable protected-route hydration. Simultaneous realtime delivery remains unproven because sequential logout/login does not keep both subscriptions open | Add a two-context or direct Supabase Realtime acceptance for live delivery, then mark complete |
