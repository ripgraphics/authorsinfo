...
| `[>]` Add authorization tests for every kind and membership transition | `IN PROGRESS` | Group chat, member listing, settings, and member-action coverage passes 26/26, including active/nonmember/suspended/event-channel access, forged sender, malformed payloads, cross-origin writes, cross-origin member removal, and database-error handling; full direct/group transition and realtime matrix remains incomplete | Add direct/group membership mutation and realtime authorization cases |
...
| Item | Status | Evidence | Next action |
| --- | --- | --- | --- |
| `[x]` Two authenticated accounts exchange direct messages without reload | `COMPLETED` for direct persistence and group realtime evidence | Sequential Chrome acceptance passes: Sam Smith sent `Sequential acceptance check 2026-09-23 Sam to Bob`, Bob Brown loaded it after account switching and sent `Sequential acceptance reply 2026-09-23 Bob to Sam`; both rendered with normal message actions. A live two-context group acceptance now keeps an authenticated observer Realtime socket subscribed, sends through the authenticated API, and receives the inserted `group_chat_messages` row; the additive Realtime migration is applied live. The proxy refreshes Supabase cookies on normal application/API requests, preventing the prior post-login `401` cascade | Continue broader read/unread, typing, presence, media, reconnect, and direct-channel realtime acceptance |
...
| `[>]` Unauthorized/blocked/removed/suspended/event/forged cases fail closed | `IN PROGRESS` | Request and Restrict route tests cover forged/nonparticipant access; event target owner/non-owner/admin/private/public/forged matrix passes 16/16; direct-message RLS denials now return sanitized 403 responses with focused send/error tests passing 15/15; live group acceptance proves active-member access, suspended/nonmember denial, removed-member denial for member list/history/send, active-member continuity, event-channel denial, and forged-sender rejection; live direct acceptance now proves both participants receive sanitized `403 blocked_user` history/send responses after a block and regain history access after unblock. Unreachable external rate-limit providers now fail open within 750ms instead of blocking valid sends. Broader direct removed/suspended acceptance remains incomplete | Verify live direct removed/suspended cases and broader authenticated browser acceptance |
...
- **2026-09-21:** Hardened the blocked direct-message failure boundary: PostgreSQL row-level-security insert failures now return a sanitized 403 instead of a generic 500 or misleading 400. Focused direct-send and error-handler tests pass 15/15; live blocked-user acceptance remains gated by the existing Supabase auth/session and `blocks` policy verification.
- **2026-09-23:** Added same-origin protection to Messenger group member removal before invoking the existing authenticated policy action. Combined group chat, member listing, settings, and member-action suites pass 26/26; live two-account transition and realtime acceptance remain open.
- **2026-09-23:** Added live two-account group realtime acceptance and applied `20260923130000_group_chat_realtime_delivery.sql` with full replica identity and publication protection. The observer socket explicitly authenticates, reaches `SUBSCRIBED`, and receives the sender's persisted group message after the authenticated API returns `201`; the focused Playwright acceptance passes. Sequential direct messaging and simultaneous group realtime evidence are now complete; broader interaction acceptance remains open.
- **2026-09-23:** Added live removed-member transition acceptance. A disposable observer membership is removed through the live database, then member-list/history/send requests return `403` while the active member continues to receive `200`; the focused Playwright acceptance passes.
- **2026-09-23:** Bounded the external rate-limit provider call to 750ms and preserved fail-open behavior when the provider is unavailable. The focused timeout regression passes, preventing unreachable Upstash configuration from blocking group message sends.
- **2026-09-23:** Added live two-account direct blocking acceptance and fixed the RLS-hidden participant response path. Both participants now receive sanitized `403 blocked_user` history/send responses while blocked, and the blocker regains history access after the live unblock mutation; focused direct suites pass 15/15.
- **2026-09-23:** Prevented duplicate Messenger unread polling by making the full-page workspace the single poll owner and disabling the poll in the compact launcher instance. This removes the repeated inbox/history/read-state request storm that saturated the development server; focused Messenger/auth tests pass 23/23 and the production build passes with 221 routes. Also corrected the isolated `PanelLeftOpen` icon reference in the user-edited Messenger header so the build can complete.
