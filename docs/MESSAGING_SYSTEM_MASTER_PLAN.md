...
| `[>]` Unauthorized/blocked/removed/suspended/event/forged cases fail closed | `IN PROGRESS` | Request and Restrict route tests cover forged/nonparticipant access; event target owner/non-owner/admin/private/public/forged matrix passes 16/16; direct-message RLS denials now return sanitized 403 responses with focused send/error tests passing 15/15; live blocked/removed/suspended acceptance and authenticated browser acceptance remain incomplete | Verify live blocked/removed/suspended cases after Supabase auth/session stability returns |
