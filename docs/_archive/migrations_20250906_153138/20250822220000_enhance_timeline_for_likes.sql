-- =============================================================================
-- MIGRATION: Enhance Timeline Functions for Likes
-- =============================================================================
-- 
-- This migration enhances the timeline functions to properly display like activities
-- and integrate them into timeline feeds for followers and entity timelines.
--
-- =============================================================================

-- Update the entity timeline function to include like activities
    ...
    SELECT 
        ...
        CASE 
            WHEN a.activity_type = 'like' THEN 
                a.user_name || ' liked ' || COALESCE(a.data->>'liked_activity_content', 'a post')
...
-- Update the user feed function to include like activities
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(a.share_count, 0)::integer as share_count,
        COALESCE(a.view_count, 0)::integer as view_count,
        EXISTS (SELECT 1 FROM public.activity_likes l WHERE l.activity_id = a.id AND l.user_id = p_user_id)::boolean as is_liked,
        COALESCE(a.entity_type, 'user')::text as entity_type,
        COALESCE(a.entity_id::text, a.user_id::text) as entity_id,
        -- Enhanced columns with like activity support
        CASE 
