-- FIX ENGAGEMENT PERSISTENCE ISSUE - SKIP EXISTING COLUMNS
-- This script adds missing engagement columns and creates triggers to keep counts synchronized
-- Date: 2025-01-06
-- Modified to skip columns that already exist

-- =====================================================
-- STEP 1: ADD ONLY MISSING ENGAGEMENT COLUMNS TO ACTIVITIES TABLE
-- =====================================================
...
DO $$ BEGIN
    ...
    ELSE
        RAISE NOTICE 'comment_count column already exists - skipping';
    END IF;
END $$;
...
DO $$ BEGIN
    ...
    ELSE
        RAISE NOTICE 'share_count column already exists - skipping';
    END IF;
END $$;
...
DO $$ BEGIN
    ...
    ELSE
        RAISE NOTICE 'bookmark_count column already exists - skipping';
    END IF;
END $$;
...
DO $$ BEGIN
    ...
    ELSE
        RAISE NOTICE 'user_has_reacted column already exists - skipping';
    END IF;
END $$;
...
-- Create indexes for the new columns (IF NOT EXISTS handles duplicates)
