...
def main():
    ...
    print(f"Found {len(tables_data)} tables in CSV")
    ...
    sql_lines.append(f'-- This script will recreate all {len(tables_data)} tables from your original schema')
    sql_lines.append('')
    ...
    with open('complete_restore_fixed.sql', 'w', encoding='utf-8') as f:
        f.write('\n'.join(sql_lines))
    
    print(f"Generated complete_restore_fixed.sql with all {len(tables_data)} tables")
