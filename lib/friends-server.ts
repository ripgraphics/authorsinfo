...
export async function getFriends(userId: string, page = 1, limit = 100) {
  ...
  const friendUserIds = friends.map((friend: UserFriend) =>
  ...
  const [usersResult, profilesResult] = await Promise.all([
    ...
    supabaseAdmin.from('profiles').select('user_id, avatar_image_id').in('user_id', friendUserIds),
  ])
  ...
  if (userTargetType && friendUserIds.length > 0) {
    ...
    if (userFollowsData) {
      userFollowsData.forEach((follow: Follow) => {
  ...
  if (friendUserIds.length > 0) {
    ...
    if (userFriendsData.data) {
      userFriendsData.data.forEach((friendship: FriendRecord) => {
    ...
    if (reverseFriendsData.data) {
      reverseFriendsData.data.forEach((friendship: FriendRecord) => {
  ...
  if (friendUserIds.length > 0) {
    ...
    if (booksReadData) {
      booksReadData.forEach((progress: ReadingRecord) => {
  ...
  const friendsList = friends.map((friend: UserFriend) => {
