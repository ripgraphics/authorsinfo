...
export class ISBNdbDataCollector {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          throw new Error(
            'ISBNdb API daily limit exceeded (403). Your API key has reached its daily request limit. Please try again tomorrow or upgrade your API plan.'
          )
        }
      ...
      console.log(
        `[ISBNdbDataCollector] Using search results directly for ${booksArray.length} books (no additional API calls needed)`
      )
  ...
  async fetchBooksByYear(
    ...
    try {
      ...
      if (options.searchType === 'recent') {
        ...
        console.log(
          `[ISBNdbDataCollector] Searching books by year: ${year} using /books/a with year parameter`
        )
      }
        ...
        bookData.subjects.map(async (subjectName: string) => {
          try {
            ...
            if (subject) {
              ...
              await supabase.from('book_subjects').upsert(
                {
                  book_id: bookId,
  ...
  async bulkImportBooks(books: ISBNdbBookData[]): Promise<DataCollectionStats> {
    ...
    for (const book of books) {
      try {
        ...
        stats.totalSkipped++
        stats.errors.push(
          `Error processing book ${book.isbn}: ${error instanceof Error ? error.message : 'Unknown error'}`
        )
      }
    }
  ...
  async getCompleteBookData(bookId: string) {
    try {
      const { data, error } = await supabase
        .from('books_complete')
        .select('id, title, isbn, isbn13, publisher, language, date_published, overview, image, authors, subjects, created_at, updated_at')
        .eq('id', bookId)
        .single()
  ...
  async logSyncActivity(bookId: string, syncType: string, syncStatus: string, details: any = {}) {
    try {
      await supabase.from('isbndb_sync_log').insert({
        book_id: bookId,
        sync_type: syncType,
