...
-- This script adds essential enterprise features to the activities table
...
-- Step 2: Add constraints for new fields
ALTER TABLE public.activities 
ADD CONSTRAINT IF NOT EXISTS check_publish_status_values 
CHECK (publish_status IN ('draft', 'scheduled', 'published', 'archived', 'deleted'));

ALTER TABLE public.activities 
ADD CONSTRAINT IF NOT EXISTS check_bookmark_count_positive 
CHECK (bookmark_count >= 0);

ALTER TABLE public.activities 
ADD CONSTRAINT IF NOT EXISTS check_trending_score_range 
...
SELECT 
    'Activities table upgraded successfully' as status,
    COUNT(*) as total_activities,
    COUNT(CASE WHEN publish_status = 'published' THEN 1 END) as published_activities,
    COUNT(CASE WHEN is_featured = true THEN 1 END) as featured_activities
