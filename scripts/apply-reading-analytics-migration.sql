-- Migration: 20251226_phase_3_reading_analytics.sql
-- This script creates the reading_challenges and reading_sessions tables
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reading_challenges' AND column_name = 'challenge_year') THEN
        ALTER TABLE reading_challenges ADD COLUMN challenge_year INTEGER;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reading_challenges' AND column_name = 'start_date') THEN
        ALTER TABLE reading_challenges ADD COLUMN start_date TIMESTAMPTZ;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reading_challenges' AND column_name = 'end_date') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reading_sessions' AND column_name = 'session_date') THEN
        ALTER TABLE reading_sessions ADD COLUMN session_date TIMESTAMPTZ;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reading_sessions' AND column_name = 'duration_minutes') THEN
