...
async function checkImageUrl(imageUrl: string) {
  ...
  try {
    ...
    if (profilesError) {
      ...
      for (const profile of profiles) {
        ...
        if (user) {
          ...
          console.log(
            `   ⚠️  Profile ${profile.id} linked to user ${profile.user_id} (user not found)`
          )
        }
      }
    } else {
    ...
    const { data: albumImages, error: albumImagesError } = await supabase
      .from('album_images')
      .select(
        'id, entity_id, entity_type_id, image_id, is_cover, is_featured, created_at, album_id'
      )
    ...
    if (image) {
      console.log(
        `${profiles && profiles.length > 0 ? '✅' : '❌'} Linked to user profile: ${profiles && profiles.length > 0 ? `YES (${profiles.length} profile(s))` : 'NO'}`
      )
      console.log(
        `${albumImages && albumImages.length > 0 ? '✅' : '❌'} In album_images: ${albumImages && albumImages.length > 0 ? `YES (${albumImages.length} entry/entries)` : 'NO'}`
      )
      console.log(
        `${coverProfiles && coverProfiles.length > 0 ? '✅' : 'ℹ️ '} Used as cover image: ${coverProfiles && coverProfiles.length > 0 ? `YES (${coverProfiles.length} profile(s))` : 'NO'}`
      )
      ...
      if (
        ...
        (image.metadata as any).entity_id
      ) {
...
if (!imageUrl) {
  ...
  console.log('\nExample:')
  console.log(
    '  npx tsx scripts/check-image-url.ts "https://res.cloudinary.com/dj8yugwyp/image/upload/v1765837339/authorsinfo/avatar/vc8x5qtnfmcyzgkwi5va.webp"'
  )
  process.exit(1)
}
