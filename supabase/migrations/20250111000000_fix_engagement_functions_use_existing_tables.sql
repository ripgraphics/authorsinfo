-- Fix engagement functions to use existing tables
-- Update get_entity_engagement to use 'likes' and 'comments' tables instead of non-existent 'engagement_likes' and 'engagement_comments'
...
BEGIN
    RETURN QUERY
    SELECT 
        -- Use likes table for all entity types (activity_likes doesn't exist in database)
...
-- Update toggle_entity_like to use 'likes' table for all entity types (activity_likes doesn't exist)
...
-- Update add_entity_comment to use 'comments' table for all entity types (activity_comments may not exist)
